...
//this is for R2D2 tone
unsigned long startTimeStamp = 0;
float rands = 0;
int press_time = 3; //press for at least 3 seconds to activate startup;
bool start = 0; //power indicator. don't touch this.
...
byte he[8] = {0x97, 0x94, 0x94, 0xf7, 0x94, 0x94, 0x97, 0x00};
byte ey[8] = {0xa2, 0x22, 0x14, 0x88, 0x08, 0x08, 0x88, 0x00};
byte by[8] = {0xf2, 0x8a, 0x89, 0xf0, 0x88, 0x88, 0xf0, 0x00};
byte ye[8] = {0x2f, 0x28, 0x48, 0x8f, 0x88, 0x88, 0x8f, 0x00};
...
void setup() {
  ...
  for (byte i = 0; i < 6; i++) { //power on tone
    digitalWrite(buzzer, !digitalRead(buzzer)); delay(100);
  }
  startup_sequence();
  ...
  Serial.begin(9600);
  randomSeed(analogRead(0));//for creating randomness of R2D2
